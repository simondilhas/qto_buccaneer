...
def get_storey_files(graph_path):
    ...
    if os.path.exists(graph_path):
        for file in os.listdir(graph_path):
            if file.startswith("floor_layout_by_sia_"):
                if file.lower().endswith(('.png', '.jpg', '.jpeg', '.json')):
                    # Extract storey from filename - everything after 'floor_layout_by_sia_'
                    match = re.search(r'floor_layout_by_sia_(.+?)(?:\.\w+)?$', file)
                    if match:
